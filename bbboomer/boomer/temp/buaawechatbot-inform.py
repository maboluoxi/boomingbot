...
def thread_handle_message(wx_inst):
    while True:
        ...
        types = message.get('type')
        if types is not None and 'msg' in types:
            # 这里是判断收到的是消息 不是别的响应
            ...
            if data_type == '1' and (from_member_wxid == 'wxid_on6vrlm6mi4712' or from_member_wxid == 'wxid_kebeinngdjjr21'):
                ...
                if msg in member_dict.keys():
                    ...
                    wx_inst.send_text(to_user='19442513534@chatroom', msg='请%s同学关注导员的最新通知 '%(msg), at_someone=wxid)
                    # wx_inst.send_text(to_user='23850395431@chatroom', msg='请%s同学打卡[炸弹][烟花]'%(msg), at_someone=wxid)
                    time.sleep(1)
